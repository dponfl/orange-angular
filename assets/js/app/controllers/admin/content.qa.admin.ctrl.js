(function () {
  ...
  angular
    .module('OrangeClient')
    .controller('ContentQAAdminCtrl', ContentQAAdminCtrl);

  ContentQAAdminCtrl.$inject = ['GeneralConfigService', 'ContentService',
    'EditObjectService', '$log', '$rootScope', '$scope', '$q',
    'lodash', 'FileUploader', 'toaster', '$timeout', '$http'];

  /* @ngInject */
  function ContentQAAdminCtrl(GeneralConfigService, ContentService,
                              EditObjectService, $log, $rootScope, $scope, $q,
                              lodash, FileUploader, toaster, $timeout, $http) {
    var vm = this;
    var name = 'ContentQAAdminCtrl';
    var _ = lodash;
    ...
    function _setDataInInitialState() {
      ...
      for (var i = 0; i < $rootScope.numLang; i++) {
        ...
        vm.formData.langContent[useLang] = $rootScope.orangeConfig.contentQA[useLang];
      }
      ...
      $q.all(someObj)
        .then(function (results) {
          ...
          if (results.record_en.status == 200) {
            ...
            toaster.pop({
              type: 'success',
              title: __.t('ADMIN_QA_SUCCESS_TITLE'),
              body: __.t('ADMIN_QA_SUCCESS_BODY_1'),
              toasterId: '111111',
            ...
            toaster.pop({
              type: 'error',
              title: __.t('ADMIN_QA_ERROR_TITLE'),
              body: __.t('ADMIN_QA_ERROR_BODY_1'),
              toasterId: '111111',
              showCloseButton: true,
              timeout: 15000,
            });
